...
\textbf{Nota de implementación — normalización de entrada:} Las imágenes BrainWeb son
NIfTI float32 con intensidades MRI en el rango $\approx 0$–$1368$ (unidades arbitrarias
del escáner). Dado que el algoritmo AMF opera con comparaciones de orden sobre enteros
\texttt{uint8}, la imagen se normaliza automáticamente al rango 0–255 antes de ejecutar
el filtro:
\[
  I_{\text{norm}} = \operatorname{clip}\!\left(
    \frac{I - I_{\min}}{I_{\max} - I_{\min}} \cdot 255,\; 0,\; 255
  \right) \in \mathbb{Z}^+_{[0,255]}
\]
Esta normalización preserva las relaciones de orden de intensidad (rango dinámico relativo)
y con ello el comportamiento de detección de impulsos del algoritmo.
...
\begin{itemize}
  \item El Filtro de Mediana (r=1) produce resultados muy cercanos al original a bajo
        ruido (pn3), y reduce efectivamente el grano a nivel medio-alto (pn5, pn9) con
        un suavizado moderado y uniforme en los tres planos.
  \item El Filtro de Mediana Adaptativo muestra diferencias sutiles respecto al filtro
        clásico a bajo ruido (pn3, pn5) — preservando los bordes de sustancia gris/blanca
        con mayor fidelidad — y comportamiento similar a la Mediana a alto ruido (pn9).
  \item El Filtro de Wiener Adaptativo produce la textura más homogénea y suavizada de
        los tres filtros en todos los niveles de ruido. A alto ruido (pn9) el suavizado
        es más pronunciado, ya que la varianza estimada $\nu^2$ se eleva con el ruido,
        deprimiendo la ganancia en regiones de contraste moderado.
\end{itemize}
...
diagnóstica. El Filtro de Mediana con radio 1 (ventana 3×3×3) introduce un suavizado
apenas perceptible, manteniendo la delimitación córtex/CSF y la estructura de los
giro-surcos en el plano sagital. El Filtro de Mediana Adaptativo se comporta de forma muy
similar al Mediana clásico a este nivel de ruido: al ser la densidad de impulsos baja, la
mayoría de los vóxeles pasan el Nivel B sin modificación. El Filtro de Wiener Adaptativo
produce la imagen visualmente más suave de las cuatro columnas, con textura más
homogénea en el plano coronal — comportamiento esperado del estimador MMSE cuando
$\nu^2$ incluye variabilidad natural del tejido.
...
\textbf{Análisis pn5:} A nivel de ruido medio el comportamiento diferencial entre los
métodos se acentúa ligeramente respecto a pn3. El Filtro de Mediana suprime eficazmente
los puntos de ruido aislados, manteniendo el contraste global y la continuidad de los
planos sagital y coronal. El Filtro de Mediana Adaptativo exhibe resultados visualmente
muy similares a los del Mediana clásico: a este nivel de ruido, la mayoría de los vóxeles
siguen siendo correctamente clasificados como señal limpia en el Nivel B. El Filtro de
Wiener Adaptativo produce una imagen algo más suave que los otros dos: las regiones
homogéneas (tejido blanco profundo) son más uniformes, mientras las interfaces de
alto contraste (córtex/CSF, plano sagital) conservan parte de la definición original.
...
\textbf{Análisis pn9:} A alto nivel de ruido los tres filtros reducen eficazmente el grano
visible en la imagen original. El Filtro de Mediana recupera las estructuras anatómicas
macroscópicas (lobos, ventrículos, plano de la hoz) con un suavizado moderado en los
planos sagital y coronal, con cierta pérdida de definición en los giro-surcos corticales más
finos. El Filtro de Mediana Adaptativo muestra un resultado visualmente similar al Mediana
clásico: a alta densidad de ruido el criterio del Nivel A requiere con mayor frecuencia
ampliar la ventana hasta $S_{\max}=7$, y la imagen resultante se asemeja a una mediana de
ventana media-grande en las regiones más ruidosas. El Filtro de Wiener Adaptativo produce
la imagen con textura más uniforme de las tres: el alto nivel de ruido eleva la varianza
global estimada $\nu^2$, lo que deprime la ganancia de Wiener en zonas de contraste
moderado y produce el suavizado más pronunciado, con ligera pérdida de detalle respecto
a los otros dos filtros.
...
\midrule
pn3 (3\%) & AMF / Mediana & Mediana y AMF muy similares &
  AWF produce textura más uniforme; diferencias sutiles a bajo ruido \\
pn5 (5\%) & AMF & AMF y Mediana resultados similares &
  AWF más suavizado; AMF clasifica más píxeles como señal limpia \\
pn9 (9\%) & Mediana / AMF & Los tres filtros reducen el grano eficazmente &
  AWF produce la textura más uniforme; AMF se asimila a ventana media \\
\bottomrule
\end{tabular}
