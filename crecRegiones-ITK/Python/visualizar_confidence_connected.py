...
ARCHIVOS = [
    ("confidence_iter1_radio1.nii.gz", "Iter=1 Radio=1"),
    ("confidence_iter2_radio1.nii.gz", "Iter=2 Radio=1 (original taller)"),
    ("confidence_iter3_radio1.nii.gz", "Iter=3 Radio=1"),
    ("confidence_iter3_radio2.nii.gz", "Iter=3 Radio=2"),
]
...
# Cargar las 4 segmentaciones
segmentaciones = [(etiqueta, leer_imagen(ruta)) for ruta, etiqueta in ARCHIVOS]

# Mapa de color para superponer la mascara en naranja (transparente fuera de la region)
cmap_overlay = ListedColormap([(0, 0, 0, 0), (1.0, 0.55, 0.0, 0.55)])
...
for i, (etiqueta, mascara) in enumerate(segmentaciones):
    ...
    for j, (titulo_plano, eje, indice) in enumerate(PLANOS):
        ...
        ax.imshow(corte_original, cmap="gray", vmin=0, vmax=vmax,
                  aspect="auto")
        # Superponer mascara binarizada en naranja
...
fig.suptitle("ConfidenceConnected - Comparación de iteraciones y radio", fontsize=14)
fig.savefig("mosaico_confidence_connected.png", dpi=150)
